...
class TestAdaptLLM:
    ...
    def __with_structured_output_support[T](
        ...
        This helper method ensures that the TypedModelStrategy properly adapts
        a mocked LLM that supports the `with_structured_output` method. It
        ...
        Args:
            ...
            manipulate mocked objects.
            model (type[T]): The target model type to which the LLM output will
            be adapted. response_lambda (Callable[[str], T]): A lambda function
            providing the structured output based on the input prompt.
